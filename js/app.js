...
document.getElementById('recordForm').addEventListener('submit', async function(e) {
  ...
  try {
    ...
    _records.unshift(saved);  // prepend to cache
    showToast('✅ 記錄已儲存');
    this.reset();
...
function updateDashboard() {
  ...
  // Distinct casinos played (no sum across casinos — each casino has different points system)
  ...
  document.getElementById('stat-casino-count').textContent  = distinctCasinos.length;
  const wlEl = document.getElementById('stat-total-winloss');
  wlEl.textContent = (totalWL >= 0 ? '+' : '') + totalWL.toLocaleString();
  wlEl.className   = 'stat-value ' + (totalWL > 0 ? 'win' : totalWL < 0 ? 'lose' : '');
  document.getElementById('stat-avg-bet').textContent     = avgBet.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  document.getElementById('stat-win-count').textContent   = winCount;
...
function updatePromoDashboard() {
  ...
  const otherCount  = promos.filter(p => ['transport','gift','spa','other'].includes(p.category)).length;
  ...
  document.getElementById('pstat-count').textContent   = promos.length;
  document.getElementById('pstat-points').textContent   = Math.round(totalPoints).toLocaleString();
  document.getElementById('pstat-fb').textContent       = fbCount;
  document.getElementById('pstat-room').textContent     = roomCount;
  document.getElementById('pstat-credit').textContent   = creditCount;
  document.getElementById('pstat-casinos').textContent  = distinctCasinos.length;
